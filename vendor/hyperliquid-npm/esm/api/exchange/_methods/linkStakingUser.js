...
const LinkStakingUserParameters = /* @__PURE__ */ (() => {
    return v.omit(v.object(LinkStakingUserRequest.entries.action.entries), ["type", "signatureChainId", "hyperliquidChain", "nonce"]);
})();
 ...
 * @see https://hyperliquid.gitbook.io/hyperliquid-docs/trading/fees#staking-linking
 */
export function linkStakingUser(config, params, opts) {
    const action = parse(LinkStakingUserParameters, params);
    return executeUserSignedAction(config, { type: "linkStakingUser", ...action }, LinkStakingUserTypes, opts);
}
