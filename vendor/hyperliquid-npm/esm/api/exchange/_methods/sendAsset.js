...
const SendAssetParameters = /* @__PURE__ */ (() => {
    return v.omit(v.object(SendAssetRequest.entries.action.entries), ["type", "signatureChainId", "hyperliquidChain", "nonce"]);
})();
 ...
 * @see https://hyperliquid.gitbook.io/hyperliquid-docs/for-developers/api/exchange-endpoint#send-asset
 */
export function sendAsset(config, params, opts) {
    const action = parse(SendAssetParameters, params);
    return executeUserSignedAction(config, { type: "sendAsset", ...action }, SendAssetTypes, opts);
}
