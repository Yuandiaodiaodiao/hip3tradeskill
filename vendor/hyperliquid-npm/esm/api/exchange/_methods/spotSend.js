...
const SpotSendParameters = /* @__PURE__ */ (() => {
    return v.omit(v.object(SpotSendRequest.entries.action.entries), ["type", "signatureChainId", "hyperliquidChain", "time"]);
})();
 ...
 * @see https://hyperliquid.gitbook.io/hyperliquid-docs/for-developers/api/exchange-endpoint#core-spot-transfer
 */
export function spotSend(config, params, opts) {
    const action = parse(SpotSendParameters, params);
    return executeUserSignedAction(config, { type: "spotSend", ...action }, SpotSendTypes, opts);
}
